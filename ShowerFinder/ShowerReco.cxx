...
if( (wdist<hlimit)&&(wdist>0.2)){ 
	...
	std::cout << " charge calc, set " << set << " " << fNPitch[set][set] << " " << wdist << " " << ch_adc << " MeV/cm: " << dEdx << " " << " av up to now: MeV " <<  fChargeMeV_4cm[set]/fNpoints_4cm[set] << " 2cm test: " << fChargeADC_2cm[set] << " " << fChargeMeV_2cm[set] << "av up to now: MeV " << fChargeMeV_2cm[set]/fNpoints_2cm[set] << "hit: wire, time " << wire << " " << time << " " << rdist << " " << fTime_vertex[plane] <<   std::endl;
 ...
 std::cout << " ------------ set nr " << set << " - hit-Vertex=" << std::endl;
	...
	std::cout << " -----------NPoints 2cm, 4cm " << fNpoints_2cm[set] << " " << fNpoints_4cm[set] <<   " average De/Dx, charge:  " << fChargeADC_2cm[set]/fNpoints_2cm[set] << " " <<
	fChargeADC_4cm[set]/fNpoints_4cm[set] << " " << fChargeMeV_2cm[set]/fNpoints_2cm[set]
	<< " " << fChargeMeV_4cm[set]/fNpoints_4cm[set] << " " << std::endl;
 ...
 //second loop to calculate RMS      
  for(art::PtrVector<recob::Hit>::const_iterator hitIter = hitlist.begin(); hitIter != hitlist.end();  			hitIter++){
    ...
    if(fNpoints_2cm[set]>0)	{
      std::cout << " ####### Npoints " << fNpoints_2cm[set] << " " << fNpoints_4cm[set] <<   " RMS, ADC:  " << TMath::Sqrt(RMS_ADC_2cm/fNpoints_2cm[set]) << " " <<TMath::Sqrt(RMS_ADC_4cm/fNpoints_4cm[set]) << " MeV: " << TMath::Sqrt(fRMS_2cm[set]/fNpoints_2cm[set])
 ...
 /// third loop to get only points inside of 1RMS of value.      
  ...
  for(art::PtrVector<recob::Hit>::const_iterator hitIter = hitlist.begin(); hitIter != hitlist.end();  			hitIter++){
   ...
   if(fNpoints_corr_MeV_2cm[set]>0)	{
	std::cout << " ++ NPoints 2cm, ADC and MeV " << fNpoints_corr_MeV_2cm[set] << " " << fNpoints_corr_ADC_2cm[set] <<   " corrected average De/Dx, charge, MeV:  " << fCorr_Charge_2cm[set]/fNpoints_corr_ADC_2cm[set] << " " <<
   ...
   if(fNpoints_corr_MeV_4cm[set]>0)	{
	std::cout << " ++ NPoints 4cm, ADC and MeV " << fNpoints_corr_MeV_4cm[set] << " " << fNpoints_corr_ADC_4cm[set] <<   " corrected average De/Dx, charge, MeV:  " << fCorr_Charge_4cm[set]/fNpoints_corr_ADC_4cm[set] << " " <<
...
void shwf::ShowerReco::GetPitchLength(unsigned int set){
 ...
 art::ServiceHandle<geo::Geometry> geom;
  ...
  for(unsigned int pl=0;pl<fNPlanes;pl++)  {
    ...
    std::cout << "++++++ calculating  -Pitch[" <<set <<"]["<<pl<<"] =" <<fNPitch[set][pl] << std::endl;
