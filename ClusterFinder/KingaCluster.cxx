 ...
 //********************************************************************     
  ...
  for(unsigned int cstat = 0; cstat < geom->Ncryostats(); ++cstat){
    for(unsigned int tpc = 0; tpc < geom->Cryostat(cstat).NTPC(); ++tpc){
      mf::LogInfo("KingaCluster")<<"No of planes = "<<geom->Cryostat(cstat).TPC(tpc).Nplanes();
...
void cluster::KingaCluster::AngularDistribution(unsigned int cstat, unsigned int tpc, unsigned int plane){   
   ...
   unsigned int p=plane, t=tpc, c = cstat;
   ...
   for(unsigned int i = 0; i< allhits.size(); ++i){
     ...
     ///\todo Need to change plane == 0,1 tests to be detector independent
     if (plane==0 ) {
...
//..............................................................   
void cluster::KingaCluster::FindMax(unsigned int cstat, unsigned int tpc, unsigned int plane){  
...
void cluster::KingaCluster::FindClusters(unsigned int cstat, unsigned int tpc, unsigned int plane){ 
