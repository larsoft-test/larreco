# Configuration file for LineMerger
#
...
process.maxEvents = clusterfinder.untracked.PSet(
    input = clusterfinder.untracked.int32(10) # See if this works to run fewer than are in input file.
)
...
process.TFileService = clusterfinder.Service(
    "TFileService",
    fileName = clusterfinder.string("linemerger_hist.root"),
...
process.source = clusterfinder.Source("PoolSource",
                                fileNames = clusterfinder.untracked.vstring("./Kinga_Events/testARTevents.root")
                                )

process.caldataCal = clusterfinder.EDProducer(
    "CalWire",
    DigitModuleLabel  = clusterfinder.string("source"),
...
process.linemerger = clusterfinder.EDProducer(
    "LineMerger",
    ClusterModuleLabel       = clusterfinder.string("hough"),
    Slope                    = clusterfinder.double(1.5),
    Intercept                = clusterfinder.double(102.0)
    )	

# Write the events to the output file.
process.output = clusterfinder.OutputModule(
    "PoolOutputModule",
    fileName = clusterfinder.untracked.string('linemerger.root'),
)

####### End of the section that defines and configures modules.#########

# Tell the system to execute all paths. Services, source, output are implied ....
process.doit = clusterfinder.EndPath(process.caldataCal*process.ffthit*process.dbscan*process.hough*process.linemerger*process.output )
