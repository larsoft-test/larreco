...
void cluster::KingaClusterAna::analyze(const art::Event& evt)
{
  ...
  unsigned int p(0),w(0), t(0),channel(0), wirevertex, channel2;
  ...
  for(size_t tpc = 0; tpc < geom->NTPC(); ++tpc){
	...
	for(unsigned int i = 0; i< hits.size(); ++i){
	  ...
	  if(p == plane && t == tpc){
	    ...
	    if(p==0 && 
	       fabs(w*0.4-fwire_vertex[plane]*0.4)<2 && 
	       fabs(((allhits[i]->StartTime()+allhits[i]->EndTime())/2.)-ftime_vertex[0])* ftimetick *fdriftvelocity<5){
	    ...
	    if(p==0 && 
	       fabs(w*0.4-fwire_vertex[plane]*0.4)<3 && 
	       fabs(((allhits[i]->StartTime()+allhits[i]->EndTime())/2.)-ftime_vertex[0])* ftimetick *fdriftvelocity<6){
	    ...
	    if(p==0 && 
	       fabs(w*0.4-fwire_vertex[plane]*0.4)<6 && 
	       fabs(((allhits[i]->StartTime()+allhits[i]->EndTime())/2.)-ftime_vertex[0])* ftimetick *fdriftvelocity<7){
	      std::cout<<"filling Ind_eng_rectangle2"<<std::endl;
